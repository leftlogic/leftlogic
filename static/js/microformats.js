...
function MicroformatsBookmarklet() {
  ...
  function loadMFTB() {
    removeOverlay();
    $j('body').append(
      '<div id="MF_box"><img class="MF_logo" src="' +
        mf_logo +
        '" height="47" width="46" /><img title="Close window" id="MF_btnClose" style="cursor: pointer;" alt=" " src="' +
        btnUp +
        '" height="14" width="14" /><p class="MF_heading">Microformats found at:</' +
        'p><p class="MF_path">' +
        location.protocol +
        '//' +
        location.hostname +
        '</' +
        'p><div id="MF_microformats"></' +
        'div></' +
        'div>'
    );
  ...
  function styles() {
    ...
    $j('#MF_box P.MF_heading').css({
      fontSize: '1.4em',
      margin: '10px',
      marginBottom: 0,
    });
    $j('#MF_box P.MF_path').css({ marginTop: '5px', fontSize: '1em' });
    $j('#MF_box DIV.MF_card').css({
      margin: '5px',
      paddingBottom: '10px',
      borderBottom: '1px #ccc solid',
      paddingLeft: '25px',
    });
    $j('#MF_box P.MF_header').css({
      fontSize: '1.2em',
      padding: '0 10px 4px 10px',
      margin: 0,
    });
    $j('#MF_box P.MF_detail').css({
      fontSize: '9px',
      color: '#666',
      paddingLeft: '10px',
      margin: 0,
    ...
    $j('#MF_box DIV.MF_vcard').css({
      background: 'url(' + vcard_image + ') no-repeat top left',
    });
    $j('#MF_box DIV.MF_vevent').css({
      background: 'url(' + ical_image + ') no-repeat top left',
    });
  }
  ...
  function positionMicroformatBox() {
    ...
    $j('#MF_box').css({
      width: '360px',
      left: (pagesize[0] - 360) / 2 + 'px',
      top: arrayPageScroll[1] + 25 + 'px',
    });
    overlaySize();
  }
  ...
  var hCard = function(hc) {
    ...
    for (i = 0; i < keys.length; i++) {
      kv = $j('.' + keys[i], hc);
      if (kv.length) {
        hCard[keys[i]] = cleanString(kv[0].title ? kv[0].title : kv.text());
      }
    }
      ...
      for (i = 0; i < keys.length; i++) {
        if (hCard[keys[i]])
          vcard +=
            keys[i].toUpperCase() +
            ':' +
            encodeURIComponent(hCard[keys[i]]) +
            nl;
      }
  ...
  var hCalendar = function(ve) {
    ...
    if (hCal.description)
      hCal.description = hCal.description.replace(/\n/g, '\\n');
    ...
    this.show = function() {
      ...
      html +=
        ...
        '</a></p>';
      if (hCal.location)
        html += '<p class="MF_detail">' + hCal.location + '</p>';
      if (hCal.dtstart)
        html += '<p class="MF_detail">' + hCal.dtstart_str + '</p>';
      ...
      for (i = 0; i < keys.length; i++) {
        if (hCal[keys[i]])
          vcal +=
            keys[i].toUpperCase() +
            ':' +
            encodeURIComponent(hCal[keys[i]]) +
            nl;
      }
  ...
  function ie_microformat() {
    ...
    f.src =
      server +
      '/save-microformats.html?' +
      this.href.substr(this.href.indexOf('BEGIN')) +
      '#' +
      this.innerHTML;
    return false;
  }
